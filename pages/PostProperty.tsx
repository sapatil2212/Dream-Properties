...
import { ArrowLeft, Upload, X, Plus, FileText, Image as ImageIcon, Building2, MapPin, IndianRupee, Check, School, Bus, ShoppingCart, Hospital, Coffee, Map, Paperclip, ExternalLink } from 'lucide-react';
import { Button, Input, Card, Badge } from '../components/UIComponents.tsx';
...
export const PostPropertyPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    avg_price: '', launch_date: '', sizes: '', project_size: '', area_unit: 'sq.ft.',
    map_link: '',
    amenities: [] as string[],
    images: [] as string[],
    ...
    nearby_locations: [] as { name: string; type: string; distance: string; time: string }[]
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/builder/post', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {activeTab === 'basic' && (
          <Card className="p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <select 
                    ...
                    value={formData.type}
                    onChange={e => {
                      setFormData({...formData, type: e.target.value, property_subtype: ''});
                    }}
                  >
              ...
              {/* Conditional Property Sub-Type for Residential */}
              ...
              {/* Conditional Property Sub-Type for Commercial */}
              ...
              {/* Conditional Bedrooms and Bathrooms - Only for Residential */}
              {formData.type === 'Residential' && (
                <>
                  <div className="grid grid-cols-2 gap-4">
                    <Input 
        ...
        {activeTab === 'details' && (
          <Card className="p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Input label="Project Units" type="number" placeholder="203" value={formData.project_units} onChange={e => setFormData({...formData, project_units: e.target.value})} />
              <div className="space-y-1">
              ...
              <Input label="Project Area" placeholder="2.08 Acres" value={formData.project_area} onChange={e => setFormData({...formData, project_area: e.target.value})} />
              <Input label="Sizes" placeholder="1664 - 3983 sq.ft." value={formData.sizes} onChange={e => setFormData({...formData, sizes: e.target.value})} />
              <Input label="Project Size" placeholder="1 Building - 203 units" value={formData.project_size} onChange={e => setFormData({...formData, project_size: e.target.value})} />
              <Input label="Launch Date" placeholder="Apr, 2025" value={formData.launch_date} onChange={e => setFormData({...formData, launch_date: e.target.value})} />
              <Input label="Avg. Price" placeholder="7.4 K/sq.ft" value={formData.avg_price} onChange={e => setFormData({...formData, avg_price: e.target.value})} />
              <Input label="Possession Date" placeholder="Apr, 2028" value={formData.possession_date} onChange={e => setFormData({...formData, possession_date: e.target.value})} />
              ...
              <Input label="RERA ID" placeholder="P02400009392" value={formData.rera_id} onChange={e => setFormData({...formData, rera_id: e.target.value})} />
            </div>
        ...
        {activeTab === 'location' && (
          <Card className="p-6">
            ...
            <div className="space-y-6">
              ...
              />
              <p className="text-[10px] text-slate-500 font-medium italic">Tip: Use the 'Embed a map' link from Google Maps for best results.</p>
              
              <div className="pt-6 border-t border-slate-100">
                <h4 className="text-sm font-black text-slate-900 uppercase mb-4">Around This Project (Nearby)</h4>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <Input placeholder="Location Name (e.g. Scholars Academy)" value={newNearby.name} onChange={e => setNewNearby({...newNearby, name: e.target.value})} />
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Type</label>
                    <select 
                      className="w-full h-10 px-3 bg-slate-50 border border-slate-200 rounded-lg text-xs"
                  ...
                  <Input placeholder="Distance (e.g. 0.7 km)" value={newNearby.distance} onChange={e => setNewNearby({...newNearby, distance: e.target.value})} />
                  <div className="flex gap-2">
                    <Input placeholder="Time (e.g. 1 min)" value={newNearby.time} onChange={e => setNewNearby({...newNearby, time: e.target.value})} />
                    <Button 
                      variant="outline" 
                      onClick={() => {
                        if (newNearby.name && newNearby.distance && newNearby.time) {
                          setFormData(prev => ({ ...prev, nearby_locations: [...prev.nearby_locations, newNearby] }));
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {formData.nearby_locations.map((item, i) => (
                    <div key={i} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100 group">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <button onClick={() => setFormData(prev => ({ ...prev, nearby_locations: prev.nearby_locations.filter((_, idx) => idx !== i) }))}>
                          <X size={14} className="text-slate-400 hover:text-rose-500" />
                        </button>
                      </div>
                    </div>
        ...
        {activeTab === 'attachments' && (
          <Card className="p-6">
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              <Input placeholder="File name (e.g. Floor Plan.pdf)" value={newAttachment.name} onChange={e => setNewAttachment({...newAttachment, name: e.target.value})} />
              <Input placeholder="URL" value={newAttachment.url} onChange={e => setNewAttachment({...newAttachment, url: e.target.value})} />
              <div className="flex gap-2">
                <Input placeholder="Size (e.g. 2.4 MB)" value={newAttachment.size} onChange={e => setNewAttachment({...newAttachment, size: e.target.value})} />
                <Button 
                  variant="outline" 
                  onClick={() => {
