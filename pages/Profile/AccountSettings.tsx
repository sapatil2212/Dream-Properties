...
import { User, Mail, Lock, ShieldCheck, ArrowRight, Check, Key, Shield, ArrowLeft, ShieldAlert, Phone } from 'lucide-react';
import { Input, Button, Modal } from '../../components/UIComponents.tsx';

export const AccountSettings: React.FC<{ user: any, onUpdateUser: (user: any) => void }> = ({ user, onUpdateUser }) => {
  console.log('=== ACCOUNT SETTINGS COMPONENT RENDER ===');
  ...
  useEffect(() => {
    console.log('=== ACCOUNT SETTINGS useEffect TRIGGERED ===');
    console.log('User prop changed:', user);
    if (user) {
      console.log('Setting state - Name:', user.name);
      console.log('Setting state - Email:', user.email);
      console.log('Setting state - Mobile:', user.mobile);
      console.log('Mobile is truthy?', !!user.mobile);
      console.log('Mobile is empty string?', user.mobile === '');
      ...
      console.log('User prop is null/undefined');
    }
    console.log('==========================================');
  }, [user]);
  ...
  const handleVerifyCurrentPassword = async () => {
    ...
    setPasswordError('');
    try {
      const response = await fetch('http://localhost:5000/api/profile/verify-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const handleRequestEmailChange = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:5000/api/profile/request-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <Button onClick={handleUpdateName} className="px-8 rounded-xl font-black uppercase text-[10px] tracking-widest">
                Save Changes
              </Button>
            </div>
          </section>
