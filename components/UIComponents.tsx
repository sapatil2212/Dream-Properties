...
import { X, ChevronDown, Inbox, Loader2 } from 'lucide-react';
...
export const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { label?: string, icon?: React.ReactNode }> = ({ label, icon, className = '', ...props }) => (
  <div className="flex flex-col gap-1.5 w-full">
    {label && <label className="text-[10px] font-black uppercase tracking-[0.1em] text-slate-500">{label}</label>}
    <div className="relative">
      ...
      <input
        className={`px-4 py-2.5 rounded-xl border border-slate-200 bg-white transition-all focus:border-blue-500 outline-none disabled:bg-slate-50 text-sm font-medium w-full ${icon ? 'pl-11' : ''} ${className}`}
        {...props}
      />
    </div>
  </div>
    ...
    <div className={`relative ${className}`} ref={containerRef}>
      <button
        ...
        className={`w-full flex items-center justify-between bg-white border border-slate-200 rounded-xl font-bold text-slate-700 focus:outline-none transition-all ${
          size === 'sm' ? 'px-3 py-1.5 text-[10px]' : 'px-4 py-2.5 text-sm'
        }`}
      >
        <span className="truncate">{selectedOption ? selectedOption.label : placeholder}</span>
        <ChevronDown size={size === 'sm' ? 12 : 16} className={`ml-2 text-slate-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>
